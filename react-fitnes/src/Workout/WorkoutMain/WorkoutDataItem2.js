...
export default function WorkoutDataItem2({ setStore, store }) {
    ...
    function dataWork () {        
        if(count === 1) {
            ...
            store.days2[0].position = true;
        } else if(count === 2) {
            for(let i = 0; i < 7; i++) {
                store.days1[i].position = false;
                store.days2[i].position = false;
            }
            store.days2[1].position = true;
        } else if(count === 3) {
            for(let i = 0; i < 7; i++) {
                store.days1[i].position = false;
                store.days2[i].position = false;
            }
            store.days2[2].position = true;
        } else if(count === 4) {
            for(let i = 0; i < 7; i++) {
                store.days1[i].position = false;
                store.days2[i].position = false;
            }
            store.days2[3].position = true;
        } else if(count === 5) {
            for(let i = 0; i < 7; i++) {
                store.days1[i].position = false;
                store.days2[i].position = false;
            }
            store.days2[4].position = true;
        } else if(count === 6) {
            for(let i = 0; i < 7; i++) {
                store.days1[i].position = false;
                store.days2[i].position = false;
            }
            store.days2[5].position = true;
        } else if(count === 7) {
            for(let i = 0; i < 7; i++) {
                store.days1[i].position = false;
                store.days2[i].position = false;
            }
            store.days2[6].position = true;
        }
